  ...
  <BCard no-body>
    <BCardHeader class="bg-light-subtle border-light p-3 d-flex align-items-center">
      ...
      <div>
        ...
        <span :class="clsx('badge fs-xxs badge-label', project.status === 'delayed' ? 'badge-soft-danger' : project.status === 'pending-review' ? 'badge-soft-warning' : 'badge-soft-success')">
          {{ toPascalCase(project.status) }}
        </span>
      </div>

      <div class="ms-auto">
        <BDropdown placement="bottom-end" :variant="null" toggle-class="btn btn-icon btn-ghost-light text-muted drop-arrow-none">
...
import { BCard, BCardBody, BCardHeader, BCol, BDropdown, BDropdownItem, BProgress, BRow } from 'bootstrap-vue-next'
