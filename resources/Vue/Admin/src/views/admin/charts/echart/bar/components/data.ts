...
export const getStackedBar = (): EChartsOption => ({
  ...
  tooltip: {
    ...
    border-bottom: 1px solid ${getColor('default-300')}; margin-bottom: 8px;
...
const transformData = (dataSet: Record<string, number[]>): YearlyData => {
  ...
  for (const year of Object.keys(dataSet)) {
    ...
    if (!values) continue // skip undefined years
    result[year] = values.map((value, index) => ({
      name: months[index] ?? 'N/A',
      value,
    }))
  }
  return result
  ...
  baseOption: {
    ...
    series: [
      ...
      },
      {
        ...
        barWidth: '15px',
        barGap: '0.25',
        itemStyle: { color: getColor('chart-info'), borderRadius: [3, 3, 0, 0] },
      },
      {
        ...
        barWidth: '15px',
        barGap: '0.25',
        itemStyle: { color: getColor('chart-warning'), borderRadius: [3, 3, 0, 0] },
      },
    ],
    grid: {
      top: '10%',
