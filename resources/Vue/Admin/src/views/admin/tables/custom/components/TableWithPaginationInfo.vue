<template>
  <div class="card">
          ...
          <tr v-for="item in paginatedData" :key="item.sku">
            ...
            <td>
              <div class="flex justify-center gap-1.25">
                ...
                <button @click="deleteRow(item)" class="btn bg-danger btn-icon btn-sm text-white size-7.5 rounded-full">
    ...
    <div class="card-footer flex justify-between items-center">
      <TablePagination label="products" :currentPage="currentPage" :perPageItems="perPage" :totalItems="filteredData.length" @change-page="(page: number) => (currentPage = page)" />
    </div>
  </div>
</template>
...
// Pagination Info
const startItem = computed(() => (filteredData.value.length === 0 ? 0 : (currentPage.value - 1) * perPage.value + 1))

const endItem = computed(() => Math.min(currentPage.value * perPage.value, filteredData.value.length))
