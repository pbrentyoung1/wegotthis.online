...
const toneFor = (value: string) => {
  switch (value) {
    case "Full":
      return "bg-[#EAF2F5] text-[#25313A] border-[#BFD3DC]";
    case "Limited":
      return "bg-[#FFF4EE] text-[#25313A] border-[#F1C7B2]";
    case "View":
      return "bg-[#FBFCFD] text-[#48535A] border-[#D8E1E6]";
    default:
      return "bg-[#F3F5F6] text-[#66737B] border-[#D8E1E6]";
  }
};
  ...
  return (
    <div className="bg-[#F7F9FB] p-5 md:p-8">
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]">
        <div className="overflow-hidden rounded-[1.5rem] border border-[#D8E1E6] bg-white shadow-[0_10px_25px_rgba(37,49,58,0.05)]">
          <div className="overflow-x-auto">
            <table className="min-w-[1150px] border-collapse text-sm">
              <thead className="bg-[#25313A] text-white">
                <tr>
                  <th className="sticky left-0 bg-[#25313A] px-4 py-4 text-left font-semibold">Permission Area</th>
                  {roles.map((role) => (
              ...
              <tbody>
                {rows.map((row, rowIndex) => (
                  <tr key={row[0]} className={rowIndex % 2 === 0 ? "bg-white" : "bg-[#FBFCFD]"}>
                    <td className="sticky left-0 border-t border-[#D8E1E6] bg-inherit px-4 py-4 font-semibold text-[#25313A]">
                      {row[0]}
                    </td>
                    {row.slice(1).map((value, cellIndex) => (
                      <td key={`${row[0]}-${cellIndex}`} className="border-t border-[#D8E1E6] px-4 py-4">
        ...
        <aside className="rounded-[1.5rem] bg-[#25313A] p-6 text-white shadow-[0_12px_30px_rgba(37,49,58,0.12)]">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#A9CBD8]">Matrix Rules</p>
