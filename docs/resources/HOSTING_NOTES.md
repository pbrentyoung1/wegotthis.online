...
- Initial hosting is expected to use a hosting.com reseller account.
...
| PostgreSQL PHP extensions | Enabled | `pdo_pgsql` and `pgsql` are enabled. Actual PostgreSQL connection still needs to be tested with credentials. |
